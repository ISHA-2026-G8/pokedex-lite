...
// //  3. Replace GITHUB_CLIENT_ID below with your app's Client ID
// //  4. Set REACT_APP_GITHUB_CLIENT_ID in your .env file (recommended)
// //
// // In dev without a real Client ID, the app runs in MOCK_MODE (see below).
// // ─────────────────────────────────────────────────────────────────────────────
...
// // localStorage key for persisted session
// const SESSION_KEY = 'pokedex-auth-session';

// // ─── Mock user (shown when no real Client ID is configured) ──────────────────
// const MOCK_USER = {
//   id: 0,
//   login: 'trainer_ash',
//   name: 'Ash Ketchum',
...
//   const [authLoading, setAuthLoading] = useState(false); // during OAuth flow
...
//           // Mock sessions don't need token verification
//           if (session.mock) {
...
//   // ── Handle OAuth callback (code in URL after redirect) ───────────────────
//   useEffect(() => {
...
//     // NOTE: GitHub's token exchange requires a server because the client_secret
//     // must stay private. In a real app, POST to your own /api/auth/github/callback.
//     // Here we show the flow architecture; the actual exchange would be:
//     //
...
//     //
//     // For this proof-of-concept we fall back to mock mode after showing the
...
//     setAuthLoading(true);
//     try {
//       // ── Real flow (needs your own backend) ──────────────────────────────
//       // Uncomment and adapt when you have a backend:
//       //
//       // const res = await fetch('/api/auth/github/callback', {
//       //   method: 'POST',
//       //   headers: { 'Content-Type': 'application/json' },
...
//       // persistSession({ user: profile, token: access_token, mock: false });
//       // setUser({ ...profile, token: access_token });

//       // ── Demo fallback ────────────────────────────────────────────────────
//       // Since we can't exchange the code without a backend in this PoC,
//       // we sign in the mock user to demonstrate the post-auth UI.
//       await new Promise(r => setTimeout(r, 800)); // simulate network
//       persistSession({ user: MOCK_USER, mock: true });
//       setUser(MOCK_USER);
//     } catch (err) {
...
//     if (!GITHUB_CLIENT_ID) {
//       // No Client ID → instant mock sign-in for development
//       setAuthLoading(true);
//       setTimeout(() => {
//         persistSession({ user: MOCK_USER, mock: true });
//         setUser(MOCK_USER);
...
// // ── Utility: fetch GitHub user profile ────────────────────────────────────
...
// // ── Utility: persist session to localStorage ──────────────────────────────
