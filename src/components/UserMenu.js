...
export default function UserMenu() {
  ...
  // Close on outside click — use mousedown so it fires before button's onClick
  useEffect(() => {
    const handle = (e) => {
      if (menuRef.current && !menuRef.current.contains(e.target)) {
        setOpen(false);
  ...
  // BUG FIX: Logout must happen BEFORE closing the dropdown.
  // Calling setOpen(false) first caused a re-render that unmounted the
  // button before logout() fired. Now logout runs first, which unmounts
  // the whole UserMenu cleanly.
  const handleLogout = () => {
    logout();
  };
